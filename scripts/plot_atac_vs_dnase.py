...
def plot_crispr_pos(pdf_writer, crispr_df: pd.DataFrame, abc_regions_df: pd.DataFrame) -> None:
    ...
    fig = get_scatter(crispr_dnase_counts, crispr_atac_counts, "CRISPR Positives DNase to ATAC Signal")
    pdf_writer.savefig(fig)

    fig = get_scatter(np.log2(crispr_dnase_counts + 1), np.log2(crispr_atac_counts + 1), "CRISPR Positives Log2 DNase to Log2 ATAC Signal")
    pdf_writer.savefig(fig)

    fig = get_swarm(crispr_dnase_counts, crispr_atac_counts, "CRISPR Positives")
    pdf_writer.savefig(fig)

def plot_crispr_neg(pdf_writer, crispr_df: pd.DataFrame, abc_regions_df: pd.DataFrame) -> None:
    ...
    fig = get_scatter(crispr_dnase_counts, crispr_atac_counts, "CRISPR Negatives DNase to ATAC Signal")
    pdf_writer.savefig(fig)

    fig = get_scatter(np.log2(crispr_dnase_counts + 1), np.log2(crispr_atac_counts + 1), "CRISPR Negatives Log2 DNase to Log2 ATAC Signal")
    pdf_writer.savefig(fig)
...
def plot_random_regions(pdf_writer, abc_regions_df: pd.DataFrame) -> None:
    fig = get_scatter(abc_regions_df["atac_counts"], abc_regions_df["dnase_counts"], "Random Regions DNase to ATAC Signal")
    pdf_writer.savefig(fig)
...
def count_coverage_at_regions(abc_regions_df: pd.DataFrame, atac_bw_file: str, dnase_bw_file: str) -> pd.DataFrame:
    ...
    print(
        f"Skipping {len(regions_to_skip)} regions due to lack of coverage"
    )
